###############################################################
# OAN-Infra-Provisioning | AWS | staging
...
terraform {
  ...
  backend "s3" {
    bucket         = "oan-tfstate-aws-staging"
    key            = "aws/staging/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "oan-tfstate-lock-staging"
    encrypt        = true
  }
}
...
data "aws_secretsmanager_secret" "db_username" {
  name = "oan/aws/staging/db/username"
}
data "aws_secretsmanager_secret" "db_password" {
  name = "oan/aws/staging/db/password"
}
