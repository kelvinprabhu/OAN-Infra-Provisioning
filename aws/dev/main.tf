###############################################################
# OAN-Infra-Provisioning | AWS | dev
...
terraform {
  ...
  backend "s3" {
    bucket         = "oan-tfstate-aws-dev"
    key            = "aws/dev/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "oan-tfstate-lock-dev"
    encrypt        = true
  }
}
...
data "aws_secretsmanager_secret" "db_username" {
  name = "oan/aws/dev/db/username"
}
data "aws_secretsmanager_secret" "db_password" {
  name = "oan/aws/dev/db/password"
}
