###############################################################
# OAN-Infra-Provisioning | AWS | box
...
terraform {
  ...
  backend "s3" {
    bucket         = "oan-tfstate-aws-box"
    key            = "aws/box/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "oan-tfstate-lock-box"
    encrypt        = true
  }
}
...
data "aws_secretsmanager_secret" "db_username" {
  name = "oan/aws/box/db/username"
}
data "aws_secretsmanager_secret" "db_password" {
  name = "oan/aws/box/db/password"
}
