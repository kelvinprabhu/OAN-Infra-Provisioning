###############################################################
# OAN-Infra-Provisioning | Azure | STAGING
...
terraform {
  ...
  backend "azurerm" {
    ...
    storage_account_name = "oantfstateazurestaging"
...
module "database" {
  ...
  db_admin_login_secret    = "oan-azure-staging-db-admin-login"
  db_admin_password_secret = "oan-azure-staging-db-admin-password"
}
